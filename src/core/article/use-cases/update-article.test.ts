...
const updateOk: OutsideUpdateArticle<string> = async (data: UpdateArticle) => {
  return `Article ${data.title} successfully updated!`
}
...
it('Should create an article properly', async () => {
  return pipe(
    ...
    mapAll(result => expect(result).toBe(`Article ${data.title} successfully updated!`)),
  )()
})

it('Should create an article properly when all fields are passed', async () => {
  return pipe(
